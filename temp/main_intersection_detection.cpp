...
int main()
{
    ...
    while (true)
    {
        ...
        if (do_execute_main_task)
        {
            ...
            bool leftBranch = (leftMean > 0.5f && centerMean > 0.3f && rightMean <= 0.5f);
            bool rightBranch = (rightMean > 0.5f && centerMean > 0.3f && leftMean <= 0.5f);
            ...
            if (isIntersection)
            {
                ...
                printf("Decicion: L:%d  R:%d C:%d t:%d\n", leftBranch, rightBranch, crossRoad, isIntersection);
                motor_M1.setVelocity(0);
                motor_M2.setVelocity(0);
                for (int i = 0; i < 2; ++i)
                {
                ...
                float turn_speed = Kp * dir * (7 * M_PIf / 18.0f);
                // robot_coord = {0.5f * wheel_vel_max * r_wheel, turn_speed};
                robot_coord = {0.0f, turn_speed};
                wheel_speed = Cwheel2robot.inverse() * robot_coord;
                motor_M1.setRotation(motor_M1.getRotation() + 0.5f);
                motor_M2.setRotation(motor_M2.getRotation() + 0.5f);
                thread_sleep_for(1000);
                printf("Dir = %d\n",dir);
                if (dir == -1)
                {
                    float finalAngle = motor_M2.getRotation() + getrevolutions(90, r_wheel, b_wheel);
                    while(motor_M2.getRotation()-finalAngle>0.02){
                        motor_M1.setRotation(0);
                        motor_M2.setRotation(finalAngle);
                    }
                }
                else if (dir == 1)
                {
                    float finalAngle = motor_M1.getRotation() + getrevolutions(90, r_wheel, b_wheel);
                    while(motor_M1.getRotation()-finalAngle>0.02){

                        motor_M1.setRotation(finalAngle);
                        motor_M2.setRotation(0);
                    }
                }
                thread_sleep_for(1000);
