 ...
 *
 * Description:
 *  Complete code for following the line while overcoming 
 *      step and wave obstacles while balancing the ball
 *  Uses an IR sensor to detect upcoming obstacles
 *  Uses a lifting mechanism to lift the Line follower array
...
int main()
{
    ...
    /********** CONTROLLER TUNING **********/
    const float Kp{5.0f}; // base gain
    const float wheel_vel_max = 2.0f * M_PIf * motor_M2.getMaxPhysicalVelocity();
    const float v_max = 0.2f * wheel_vel_max * r_wheel; // maximum forward speed 0.9
    ...
    Servo servo_lift(PB_D0); // Servo to lift up the Line Follower Array sensor bar
    ...
    static float lift_servo_width = 0.48f;
    ...
    const float normalised_angle_gain = 1.5f / M_PIf; // tuned the sensitivity to roll and pitch
    const float normalised_angle_offset = 0.5f;

    /****** IR Sensor **********/
    IRSensor ir_sensor(PC_2); // for detecting when an obstacle is approached
    ir_sensor.setCalibration(8042.5280f, -274.2508f);
    // ir distance sensor
    const float ir_dist_thresh = 3.8f; // define a variable to store measurement (in mV)
    float ir_distance_cm = 0.0f;
    int lift_up_counter = 0;
    bool isLiftedUp = false;
    bool noLiftUp = false; // this prevents false positives when the robot nose-dives while dropping from the step
    ...
    while (true)
    {
        ...
        if (do_execute_main_task)
        {
            ...
            pitch_servo_width = normalised_angle_gain * rp(1) + (normalised_angle_offset - 0.12);
            ...
            if (ir_distance_cm <= ir_dist_thresh && !noLiftUp)
            {
                isLiftedUp = true;
            }

            if (!isLiftedUp) // If no obstacle is detected
            { //keep the line follower array down
                servo_lift.setPulseWidth(lift_servo_width);

                // omega = Kp * std::pow(angle, angle_exp);
                ...
                {
                    lift_up_counter++;
                    if (lift_up_counter >= 3 * 50) // false positives from the IR sensor will be prevented until after 3 seconds
                    {
                        noLiftUp = false;
                        lift_up_counter = 0;
                        // printf("Dont go Up!!!!\n");
            ...
            { // Else lift the Line follower array up
                servo_lift.setPulseWidth(lift_servo_width - 0.5);
                v = v_max;
                omega = 0;

                lift_up_counter++;
                if (lift_up_counter >= 2 * 50) // if the line follower array has been up for 2 seconds (50 cycles in a second)
                {
                    lift_up_counter = 0;
                    isLiftedUp = false; // drop the line follower array
                    noLiftUp = true; // after we drop the line follower sensor, it should not lift up again until after 3 seconds
                                    // this prevents false positives when the robot nose-dives while dropping from the step
                }
            }

            printf("Lifupcounter: %d\n", lift_up_counter);
