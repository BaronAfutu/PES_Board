 ...
 *  This solved the maze for robot prototype 3 as r_wheel
 ...
 *  This is the final code for the MAZE
 */
...
int main()
{
    ...
    const float r_wheel = 0.12f / 2.0f; // 0.035f / 2.0f;
    ...
    float v = 0.2f * wheel_vel_max * r_wheel;
    float w = Kp * angle;

    // servos
    Servo servo_lift(PB_D0); // futuba S3001
    ...
    static float lift_servo_width = 0.45f;
            ...
            case RobotState::TRAIN:
            {
                ...
                case MotionState::FOLLOWLINE:
                {
                    ...
                    if (isIntersection)
                    {
                        ...
                        // 0.10 is the number of rotation required of 35-40mm given wheel radius
                        // 0.10 = 40mm/(2*pi*r_wheel)
                        fixedAngle = motor_M1.getRotation() + 0.10;
                        finalAngle = motor_M2.getRotation() + 0.10;
                    ...
                    else if (isDeadEnd)
                    {
                        ...
                        if (deadEndCounter >= 1 * 50)
                        {
                            ...
                            fixedAngle = motor_M1.getRotation() + getrevolutions(-100, r_wheel, b_wheel); // fixed
                            finalAngle = motor_M2.getRotation() + getrevolutions(100, r_wheel, b_wheel);
                    ...
                    else
                    {
                        ...
                        deadEndCounter = 0;
                        v = 0.2f * wheel_vel_max * r_wheel;
                        w = Kp * angle; //*std::fabs(angle);
                        robot_coord = {v, w};
            ...
            case RobotState::RUN:
            {
                ...
                case MotionState::PREPARE_TURN:
                {
                    ...
                    if (std::fabs(motor_M1.getRotation() - fixedAngle) < 0.01)
                    {
                        ...
                        {
                            // RIGHT 90 DEGREE TURN
                            fixedAngle = motor_M1.getRotation() - 0.10; // other wheel pulls back slightly to adjust for turn error
                            finalAngle = motor_M2.getRotation() + getrevolutions(90, r_wheel, b_wheel);
                        ...
                        else if (dir == 'L')
                        {
                            // LEFT 90 DEGREE TURN
                            finalAngle = motor_M1.getRotation() + getrevolutions(90, r_wheel, b_wheel);
                            fixedAngle = motor_M2.getRotation() - 0.10; // other wheel pulls back slightly to adjust for turn error
