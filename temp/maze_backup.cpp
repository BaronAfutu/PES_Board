...
int main()
{
    ...
    const float r_wheel = 0.088f / 2.0f; // 0.035f / 2.0f;
    const float b_wheel = 0.180f;        // 0.157f;
    ...
    // Controller tuning // TODO use better non-linear controller so we can go fast yet make sharp turns slow
    const float Kp = 6.0f;
            ...
            case RobotState::TRAIN:
            {
                ...
                case MotionState::FOLLOWLINE:
                {
                    ...
                    if (isIntersection)
                    {
                        ...
                        fixedAngle = motor_M1.getRotation() + 0.125; // 0.5f; // should be 0.115 for the new wheels
                        finalAngle = motor_M2.getRotation() + 0.125; // 0.5f; // should be 0.115 for the new wheels
